...
namespace CrossX.Graphics
{
    public abstract class SpriteBatch<TVertex>: IDisposable where TVertex: struct
    {
        ...
        public SpriteBatch(IGraphicsDevice graphicsDevice, IObjectFactory objectFactory, VertexContent vertexContent)
        {
        ...
        public void Flush()
        {
            ...
            basicShader.SetWorldTransform(Matrix.Identity);
        ...
        public void DrawImage(Texture2D texture, RectangleF targetRect, Rectangle? srcRect, Color4 color, SpriteFlags spriteFlags = SpriteFlags.None)
        {
            ...
            var srcL = (float)srcRect1.X / (float)texture.Width;
            var srcT = (float)srcRect1.Y / (float)texture.Height;
            var srcR = (float)srcRect1.Right / (float)texture.Width;
            var srcB = (float)srcRect1.Bottom / (float)texture.Height;
    ...
    public class SpriteBatch : SpriteBatch<VertexPCT>
    {
        public SpriteBatch(IGraphicsDevice graphicsDevice, IObjectFactory objectFactory) : base(graphicsDevice, objectFactory, VertexPCT.Content)
        {
        }
    ...
    public class SpriteBatchWithNormals : SpriteBatch<VertexPNCT>
    {
        public SpriteBatchWithNormals(IGraphicsDevice graphicsDevice, IObjectFactory objectFactory) : base(graphicsDevice, objectFactory, VertexPNCT.Content)
        {
        }
